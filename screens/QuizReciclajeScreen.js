...
const QuizReciclajeScreen = ({ navigation }) => {
  ...
  const handleAnswer = (isCorrect) => {
    ...
    if (nextQuestion < questions.length) {
      ...
      saveQuizScore(); // Guarda el puntaje al completar el quizz
    }
  };
  ...
  const saveQuizScore = async () => {
    ...
    if (user) {
      ...
      // Incrementa el puntaje obtenido antes de guardarlo
      const newScore = score + 1; // Aquí se agrega +1 al puntaje
      if (newScore > currentScore) {
        await setDoc(quizRef, {
          highestScore: newScore, // Almacena el nuevo puntaje
          email: user.email,
          timestamp: new Date(),
        });
      }
  ...
  return (
    <View style={styles.container}>
      {quizCompleted ? (
        <View>
          <Text style={styles.scoreText}>Has completado el quizz. Tu puntuación: {score + 1}</Text> {/* Muestra el puntaje +1 */}
